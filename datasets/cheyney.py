...
def read_metadata(metadata_file):
    ...
    for line in lines:
        fname, text, _, _ = line.strip().split("_") #THE CHANGE
...
class Emovdb(Dataset):
    def __init__(self, keys, dir_name='/cheyney'): #THE CHANGE: dir
        self.keys = keys
        self.path = os.path.join(os.path.dirname(os.path.realpath(__file__)), dir_name)
        self.fnames, self.text_lengths, self.texts = read_metadata(os.path.join(self.path, 'angry.csv')) #THE CHANGE: last argument
        preprocess(dir_name, self) #THE CHANGE
        print('Generated mels and mags') #THE CHANGE
